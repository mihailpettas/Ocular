	...
	void Update () 
	{
		...
		if (Mathf.Abs(oldScale - particlesScale) > 0.0001f && particlesScale > 0)
		{
			...
			foreach (ParticleSystem particles in ps)
			{
				...
				m.startSpeed = new ParticleSystem.MinMaxCurve(m.startSpeed.constantMin*scale, m.startSpeed.constantMax*scale);
				m.gravityModifier = new ParticleSystem.MinMaxCurve(m.gravityModifier.constantMin*scale, m.gravityModifier.constantMax*scale);
