...
public class Reporter : MonoBehaviour
{
	...
	public void Initialize()
	{
		if (!created) {
			...
			Debug.LogWarning("tow manager is exists, deleted the second");
	...
	void drawToolBar()
	{
		...
		GUILayout.BeginArea(new Rect(0, 0, Screen.width, size.y*2));
	...
	void DrawLogs()
	{
		...
		for (int i = startIndex; (startIndex + index) < endIndex; i++) {
			...
			Rect countRect = new Rect (Screen.width - w, size.y * i, w, size.y);
	...
	void drawGraph()
	{
		...
		if (currentFrame != -1 && currentFrame < samples.Count) {
			...
			GUILayout.BeginArea(new Rect (0, Screen.height - size.y, Screen.width, size.y), backStyle);
			GUILayout.BeginHorizontal();
	...
	//new scene is loaded
	void levelLoaded(Scene previous, LoadSceneMode mode)
	{
