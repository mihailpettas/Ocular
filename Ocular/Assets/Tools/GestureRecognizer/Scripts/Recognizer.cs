...
namespace GestureRecognizer {
	...
	public class Recognizer : MonoBehaviour {
		...
		public RecognitionResult Recognize(GestureData data){
			print ("Recognize:" + Time.frameCount);
			UnityEngine.Profiling.Profiler.BeginSample ("Recognize");

			var normData = NormalizeData (data);

			UnityEngine.Profiling.Profiler.BeginSample ("Recognize.FindPattern");
		...
		private RecognitionResult findPattern(GestureData queryData){
			...
			for (int i = 0; i < patterns.Count; i++) {
				...
				foreach (var data in permutationsToLook) {
					...
					var permutScore = CalcScore (assetData, data, gestureAsset.useLinesDirections);
		...
		private Score CalcScore(GestureData data1, GestureData data2, bool useLinesDirections){
			...
			if (data1.lines.Count == data2.lines.Count) {
				...
				for (int i = 0; i < data1.lines.Count; i++) {
					...
					var scoreFwd = CalcScore (line1, line2Fwd);
					...
					if (useLinesDirections) {
						...
						var scoreBwd = CalcScore (line1, line2Bwd);
